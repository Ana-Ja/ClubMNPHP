...
class Comentario {
	...
	function salvar() {
		...
		public static function obtenerComentarios($vertodos, $idpublicacion) {
			include_once './includes/conexion.php';
			$conexion = conectar();
			$num_comentarios=0;
			//echo "Vertodos".$vertodos;
			$consulta = "Select * from comentarios where idPublicacion = '".$idpublicacion."' order by fcomentario ";
			$comentarios = array();
			...
			if ($resultado =$conexion->query($consulta1)) { //el rdo es un array con las filas en un array
			  ...
			  if ($resultado->num_rows > 0 ){
			      ...
			      while ($fila= $resultado->fetch_assoc()) {
			        ...
			        $comentario = new Comentario($fila["texto"],$idpublicacion,$fila["idUsuario"],  $fila["fcomentario"]);
			        $comentario->id = $fila["id"];
			        //en el objeto que voy a obtener de publicacion dentro del getPublicacion, los datos de
			        //usuario van a ser del Usuario del comentario, no de la publicacion, por eso le paso
			        //el idUsuario del comentario
			        $publicacion= Publicacion::getPublicacionUsuario($fila["idpublicacion"], $fila["idUsuario"]);

<?php
	...
	function guardarUsuario() {
		...
		if ($thisPost->postBlock($_POST['postID'])) {
			$token = md5(time());
			$usuario = new Usuario($_POST["nombre"],$_POST["apellidos"],$_POST["mail"],$_FILES["foto"]['name'],$_POST["pass1"], $token);
	...
	function ObtenerAmigos() {
		...
		return $contactos = Contacto::getAmigos($_SESSION['idLogeado']);
	...
	function comprobarFoto() {
			...
			if ((isset($_FILES['foto']['name'])&&($_FILES['foto']['error'] == UPLOAD_ERR_OK))) {
				...
				$name = './images/people/110/'.$mythumb100->cambioNombre(extractName($_FILES['foto']['name']))
						.extractExtension($_FILES['foto']['name']);
				...
				// $nombre = extractName($_FILES["foto"]["name"]);
				// //cambio el nombre de la imagen
				// $numero_aleatorio = md5(time());
				// $nombre_aleatorio = $numero_aleatorio.$nombre;
				// $extension = extractExtension($_FILES["foto"]["name"]);
				// $url = './images/people/110/'.$nombre_aleatorio.'.'.$extension; 
				// //con esta funcion lo que hago es optimizar la imagen
				// compress_image($_FILES["foto"]["tmp_name"], $url, 80); 
				


				//cambio el nombre de la imagen
				// $numero_aleatorio = md5(time());
				// $nombre_nuevo = $numero_aleatorio.$_FILES["foto"]["name"];
				// $nombre_nuevo_t = $numero_aleatorio.$_FILES["foto"]["name"]."_t";
			} 
	}

<?php
	...
	function comprobarFoto() {
			...
			if ((isset($_FILES['fichero']['name'])&&($_FILES['fichero']['error'] == UPLOAD_ERR_OK))) {
				...
				$mythumb100->loadImage($_FILES["fichero"]["tmp_name"]);
				$name = './images/people/110/'.$mythumb100->cambioNombre(extractName($_FILES['fichero']['name']))
						.extractExtension($_FILES['fichero']['name']);
				$mythumb100->resize($name,100, 'width');
				$mythumb100->save($name, $quality = 90);
				$mythumb50 = new thumb();
				$mythumb50->loadImage($_FILES["fichero"]["tmp_name"]);
				$name = './images/people/50/'.$mythumb50->cambioNombre(extractName($_FILES['fichero']['name']))
					."_t".extractExtension($_FILES['fichero']['name']);
